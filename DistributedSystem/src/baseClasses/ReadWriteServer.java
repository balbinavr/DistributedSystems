...
public class ReadWriteServer extends Server {

	private BufferedWriter out;
	private BufferedReader in;
	private Socket socket;
	
	public ReadWriteServer (int id){
		super(id);
	}
	...
	protected void updateValue(int value){
		try {
			//Crea un thread per la lectura i l'altra per l'escriptura del socket
	    		System.out.println(this.id+ " - Valor demanat modificat");
	            out.write(FrameUtils.createSetFrame(this.id, (this.getCurrentValue()+1)));
	            out.newLine();
	            out.flush();  
		} catch (IOException e) {
			// TODO Auto-generated catch block
